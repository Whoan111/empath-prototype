...
// ── Position card ─────────────────────────────────────────────────────────────
function PositionCard({ pos, th, stageT, T, onOpen, closed, onToggleClose }) {
  const [hov, setHov] = useState(false)
  // "active" = candidates in Interview-stage or beyond
  ...
  return (
    <div
      ...
      onClick={() => !closed && onOpen(pos)}
      style={{
        background: hov && !closed ? th.cardBgHov : th.cardBg,
        backdropFilter: th.blur, WebkitBackdropFilter: th.blur,
        border: `1px solid ${hov && !closed ? th.borderBrt : th.border}`,
        borderTop: `2.5px solid ${closed ? th.border : hov ? th.red : (hasOffer ? stageT.Offer.dot + '99' : th.border)}`,
    ...
    >
      ...
      {/* Meta row: active · days open · Close button */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <button
          ...
          style={{
            ...
            color: closed ? th.textDim : '#E90130',
            background: 'transparent',
            border: `1px solid ${closed ? th.border : '#E9013030'}`,
            borderRadius: 6, padding: '3px 10px',
            cursor: 'pointer', fontFamily: 'inherit',
            transition: 'all 0.13s', whiteSpace: 'nowrap',
          }}
          onMouseEnter={e => {
            if (!closed) { e.currentTarget.style.background = '#FFF0F2'; e.currentTarget.style.borderColor = '#E90130' }
          }}
          onMouseLeave={e => {
            if (!closed) { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.borderColor = '#E9013030' }
          }}
        >
          {closed ? 'Reopen' : 'Close'}
        </button>
